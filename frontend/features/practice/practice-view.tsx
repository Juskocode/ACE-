    ...
    <div>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_280px]">
        ...
        <aside className="space-y-4">
          <Card className="border-0 py-0 ring-1 ring-border"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-[0.1em] text-muted-foreground">Mapa da sessão</p><div className="mt-4 grid grid-cols-5 gap-2">{items.slice(0, 20).map((item, itemIndex) => <button key={`${item.id}-${itemIndex}`} onClick={() => { setIndex(itemIndex); setSelected(null); setRevealed(false); }} className={`grid aspect-square place-items-center rounded-lg text-xs font-semibold ${itemIndex === index ? 'bg-primary text-primary-foreground' : answers[item.id] === true ? 'bg-teal-100 text-teal-800' : answers[item.id] === false ? 'bg-orange-100 text-orange-800' : 'bg-muted text-muted-foreground'}`}>{itemIndex + 1}</button>)}</div></CardContent></Card>
          <Card className="border-0 py-0 ring-1 ring-border"><CardContent className="p-5"><div className="flex items-center gap-2"><Bookmark className="size-4 text-primary" /><p className="text-sm font-semibold">Proveniência</p></div><p className="mt-3 text-xs leading-5 text-muted-foreground">Questão autoral demonstrativa, revista e associada a uma fonte. Não é uma questão oficial da PNA.</p></CardContent></Card>
        </aside>
      </div>
    </div>
  );
