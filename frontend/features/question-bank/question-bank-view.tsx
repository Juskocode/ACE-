...
import { CheckCircle2, FileQuestion, Filter, Search, ShieldAlert, ShieldCheck } from 'lucide-react';
...
export function QuestionBankView({ questions }: { questions: Question[] }) {
  ...
  return (
    <div>
      <PageIntro eyebrow="Banco de questões" title="Conteúdo com contexto e origem." description="Pesquisa por área ou tema. Cada item mantém explicação, proveniência e o respetivo estado de validação." icon={FileQuestion} />
        ...
        {filtered.map((question) => (
          <Card key={question.id} className="border-0 py-0 transition-transform hover:-translate-y-0.5 hover:shadow-[0_12px_36px_rgb(31_55_52/8%)] ring-1 ring-border">
            <CardContent className="p-5 sm:p-6">
              <div className="flex flex-wrap items-center gap-2"><Badge variant="secondary">{question.area}</Badge><Badge variant="outline">{question.topic}</Badge><Badge variant="outline">{question.difficulty}</Badge><Badge className={`ml-auto ${question.status === 'Revista' ? 'bg-teal-50 text-teal-700' : 'bg-amber-50 text-amber-800'}`}>{question.status === 'Revista' ? <CheckCircle2 className="size-3" /> : <ShieldAlert className="size-3" />}{question.status}</Badge></div>
